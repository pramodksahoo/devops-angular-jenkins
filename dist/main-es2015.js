...
let AddGroupComponent = class AddGroupComponent {
    constructor(auth, userGroupService) {
        this.auth = auth;
...
let AddProductComponent = class AddProductComponent {
    constructor(productService, router, route, auth, userGroupService, spinner, snackbar) {
    ...
    ngOnInit() {
        ...
        this.userGroupService.getAllGroupsWithAllMembers().subscribe(res => {
    ...
    addProduct() {
        ...
        this.productService.addProduct(this.product_details).subscribe((res) => {
            //console.log("form data",this.product_details)
...
/* harmony import */ var ngx_cookie_service__WEBPACK_IMPORTED_MODULE_31__ = __webpack_require__(/*! ngx-cookie-service */ "./node_modules/ngx-cookie-service/ngx-cookie-service.js");
...
let AuthService = class AuthService {
    constructor(http, cookieService, snackbar, spinner) {
        this.http = http;
    ...
    isAuthenticated() {
        ...
        if (!this.jwtHelper.isTokenExpired(refresh_token) && this.jwtHelper.isTokenExpired(access_token)) {
            this.getNewToken(refresh_token).subscribe((res) => {
    ...
    user_login(email, password) {
        ...
        //           return user;
        //       }));
        return this.http.post(login_url, { email: email, password: password }, { observe: 'response' }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["tap"])(resp => {
    ...
    //     }
    create_account(first_name, last_name, email) {
        var add_user_url = this.BASE_USER_URL + 'add/';
        return this.http.post(add_user_url, { first_name: first_name, last_name: last_name, email: email }, { observe: 'response' }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["tap"])(resp => {
            //console.log('response', resp);
            return resp;
        }));
    }
    update_account_details(data) {
        ...
        return this.http.put(add_user_url, { data }, { observe: 'response', headers: headers }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["tap"])(resp => {
            console.log('response', resp);
            return resp;
        }));
    }
    ...
    setUserPassword(email, pass1, pass2) {
        var url = this.BASE_USER_URL + 'set_password/?email=' + email;
        return this.http.post(url, { password1: pass1, password2: pass2 }, { observe: 'response' }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["tap"])(resp => {
            //console.log('response', resp);
            return resp;
        }));
    }
    ...
    getGroupUserData(list) {
        ...
        let params = new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpParams"]().set("requestData", JSON.stringify({ users: list }));
...
let ConfigsService = class ConfigsService {
    ...
    saveAppsConfig(name, url) {
        ...
        return this.http.put(this.APS_CONFIG_BASE_URL, { name: name, url: url }, { headers })
...
let ConfigurationsComponent = class ConfigurationsComponent {
    constructor(configs, data, snackbar) {
    ...
    saveConfig(data) {
        console.log(data);
        this.configs.saveAppsConfig(this.selected_conf_data.name, data.confUrl).subscribe((data) => {
            //console.log(data);
...
let DashboardComponent = class DashboardComponent {
    constructor(data, route, router, auth, appsconfig, spinner, titleService) {
        this.data = data;
        this.route = route;
        this.router = router;
        this.auth = auth;
...
let EmployeeDetailsComponent = class EmployeeDetailsComponent {
    constructor(data, productService, route, auth, spinner, titleService, snackbar, userGroupService, configs) {
        this.data = data;
        this.productService = productService;
        this.route = route;
        this.auth = auth;
    ...
    ngOnInit() {
        this.auth.getGroupUserData(new Array(this.aaaa)).subscribe(data => {
            ...
            this.userGroupService.getUserAppsGroups(this.employee.email).subscribe((res) => {
                ...
                for (let group of this.user_groups) {
                    this.userGroupService.getGroupWithMembers(group).subscribe((gdata) => {
                        var data = { group_name: group, ass_emps: gdata.ass_emps.length };
                        arr.push(data);
                        this.productService.getProductByGroup(group).subscribe((res) => {
                            for (let pp of res) {
    ...
    getUserProjects(group_name, members) {
        this.productService.getProductByGroup(group_name).subscribe((res) => {
...
let EmployeesComponent = class EmployeesComponent {
    constructor(data, configs, route, router, auth, userGroupService) {
        this.data = data;
        this.configs = configs;
        this.route = route;
        this.router = router;
    ...
    getEmployees() {
        ...
        this.auth.getAllUsers().subscribe((res) => {
            //console.log(res)
            Object.values(res.data).forEach((aaa, index) => {
                this.userGroupService.getUserAppsGroups(aaa.email).subscribe((numofg) => {
...
let AddGroupModel = class AddGroupModel {
    constructor(modalService, activeModal, spinner, snackbar) {
...
let AddGroupMemberModal = class AddGroupMemberModal {
    constructor(activeModal, auth, userGroupService) {
        this.activeModal = activeModal;
        this.auth = auth;
        this.userGroupService = userGroupService;
        this.dropdownOpen = true;
...
AddGroupMemberModal.ctorParameters = () => [
    { type: _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_6__["NgbActiveModal"] },
    { type: _auth_service__WEBPACK_IMPORTED_MODULE_3__["AuthService"] },
    { type: _user_group_service__WEBPACK_IMPORTED_MODULE_5__["UserGroupService"] }
];
...
let GroupsComponent = class GroupsComponent {
    constructor(modalService, configs, route, router, auth, userGroupService, snackbar, spinner) {
        this.modalService = modalService;
        this.configs = configs;
        this.route = route;
        this.router = router;
    ...
    getMembersDataOfGroup() {
        ...
        for (let g of this.groupFromServer) {
            var list = g.ass_emps;
            this.auth.getGroupUserData(list).subscribe(res => {
                //console.log("group members user data",res);
                Object.values(res).forEach((aaa, index) => {
                    this.userGroupService.getUserAppsGroups(aaa.email).subscribe((numofg) => res[index]["groups_count"] = numofg.data.length);
                });
...
let HomeComponent = class HomeComponent {
    ...
    getData() {
        ...
        this.configs.getUserAppsConfigs(this.employee.email).subscribe(data => {
    ...
    credSubmit(formdata) {
        //console.log(formdata);
        var data;
        this.configs.saveUserCredentials(formdata.app_name, formdata.user, formdata.username, formdata.password).subscribe(data => this.data = data);
...
let LoginComponent = class LoginComponent {
    constructor(data, route, router, auth, spinner, titleService, snackbar) {
        this.data = data;
        this.route = route;
        this.router = router;
        this.auth = auth;
...
let ProductDetailsComponent = class ProductDetailsComponent {
    constructor(route, productService, spinner, snackbar, userGroupService, auth) {
        this.route = route;
        this.productService = productService;
        this.spinner = spinner;
        this.snackbar = snackbar;
...
let ProductService = class ProductService {
    ...
    addProduct(data) {
        ...
        //console.log('url@@', url);
        return this.http.post(url, data, { headers: headers, observe: 'response' }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["tap"])(resp => {
            //console.log('response', resp);
            return resp;
        }));
    }
...
let ProductComponent = class ProductComponent {
    constructor(productService, spinner, snackbar) {
        this.productService = productService;
        this.spinner = spinner;
        this.snackbar = snackbar;
    }
...
let SidebarComponent = class SidebarComponent {
    constructor(route, router, userGroupService) {
        this.route = route;
        this.router = router;
        this.userGroupService = userGroupService;
        this.isAdmin = false;
    ...
    checkAdmin() {
        let groups;
        this.userGroupService.getUserAppsGroups(this.user_data.email).subscribe((res) => {
            groups = res;
            //console.log(res)
...
let SignupComponent = class SignupComponent {
    constructor(data, auth, route, router, spinner, snackbar) {
        this.data = data;
        this.auth = auth;
        this.route = route;
        this.router = router;
...
let UserGroupService = class UserGroupService {
    ...
    getUserGroups(user) {
        ...
        return this.http.get(url + user).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])((resp) => {
            //console.log('response', resp);
...
UserGroupService.ctorParameters = () => [
    { type: _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpClient"] },
    { type: ngx_cookie_service__WEBPACK_IMPORTED_MODULE_4__["CookieService"] }
];
